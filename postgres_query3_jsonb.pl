...
getopts('H:D:p:m:n:f:c:');

if (!$opt_H || !$opt_D || !$opt_p || !$opt_m || !$opt_n || !$opt_f || !$opt_c) {
    die "Must provide options -H (hostname), -D (database name), -p (db user password), -m (protocol name), -n (num reps), -f (out filename), -c (num markers to select) \n";
}
...
for (1..$num_reps) {
    ...
    for (1..$num_markers) {
        push @randomly_select_markers, $marker_names[int rand(scalar(@marker_names))];
    }
    #print Dumper \@randomly_select_markers;

    my %selected_markers = map { $_ => 1 } @randomly_select_markers;
    ...
    while (my $stock_id = $sth_stock->fetchrow_array) {
        ...
        if ($mutations_count == $num_markers) {
            push @accessions_with_mutations, $stock_id;
        }
    }

    # print Dumper \@accessions_with_mutations;
    print join(",", @accessions_with_mutations)." are accessions with mutations in markers:  ".join(",", @randomly_select_markers)."\n";
    my $n_end = Time::HiRes::time();
    my $n_duration = $n_end - $n_start;

    print $fh  join(",", @accessions_with_mutations)." are accessions with mutations in markers:  ".join(",", @randomly_select_markers)."\t".$n_duration."\n";
